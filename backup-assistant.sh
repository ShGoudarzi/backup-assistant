...
# 1 is for just save backup file locally
# 2 is for both save locally and send to ftp server
# 3 is for just send backup file to ftp server
### Mode ###
mode="1"
...
if [ -z "$i_config_files" ]; 
then
    echo -e "$NOW Not found any source config files!"
    exit 0
fi
...
### FTP Upload
if [ "$ftp_enable" == "yes" ] && [ "$mode" != "1" ];
then
    echo -e "Uploading to FTP-server..." | log_print

    for res in ${i_result_files[@]};
    do  
        input=$(echo $G_path | sed 's/\//\\\//g')
...
if [ $G_path != $tmp_path ];
then
    echo -e "Cleaning local backups older than $local_save_last_versions old versions..." | log_print
...
if [ "$ftp_enable" == "yes" ] && [ "$mode" != "1" ];
then
    echo -e "Cleaning remote ftp backups older than $ftp_save_last_versions old versions..." | log_print
    ...
    do
        ...
        source_file=$(echo $source_file | cut -d "." -f 2)
        local_path_dir=$G_path/$source_file
